...
Daily Project -- In this project, you will create an application using the NASA MARS ROVER API that will feature stateless and stateful components.

I experienced initial challenges with rendering actions in the appropriate order once additional child components were added. After some adjustments and a slight restructuring of the ImageDisplay and GetImageButton components, I was able to successfully gather user input and render the appropriate images on the screen. NOTE: If the images do not appear for the selected dates, it is because the database does not have data stored for that day (next update will include error messaging for this scenario)
